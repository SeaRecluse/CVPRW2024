...
from datasets import build_dataset,split_dataset
...
def get_args_parser():
    ...
    parser.add_argument('--train_data_ratio', default=None, type=float,
                        help='train data ratio in all data')


    # Model parameters
    parser.add_argument('--model', default='convnextv2_base.fcmae_ft_in22k_in1k', type=str, metavar='MODEL',
...
def main(args):
    ...
    dataset_val_part, dataset_val = split_dataset(dataset_val , args.train_data_ratio, args=args)
    dataset_train, dataset_train_part = split_dataset(dataset_train , args.train_data_ratio, args=args)

    dataset_train = ConcatDataset([dataset_train, dataset_val_part])  # 在这个列表中添加所有的数据集
    dataset_val = ConcatDataset([dataset_train_part, dataset_val])  # 在这个列表中添加所有的数据集
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats = evaluate(data_loader_val, model, device, use_amp=args.use_amp)
            print(f"acer of the model on the {len(dataset_val)} test images: {test_stats['acer']:.3f}%")
            if min_acer > test_stats["acer"]:
                min_acer = test_stats["acer"]
                max_acc = test_stats["acc1"]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="best", model_ema=model_ema)
            elif min_acer == test_stats["acer"] and max_acc <= test_stats["acc1"]:
                min_acer = test_stats["acer"]
                max_acc = test_stats["acc1"]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="best", model_ema=model_ema)
            print(f'Min acer: {min_acer:.3f}% acc1: {max_acc:.3f}% ')

            if log_writer is not None:
                log_writer.update(test_acc1=test_stats['acc1'], head="perf", step=epoch)
                log_writer.update(test_acc5=test_stats['acc5'], head="perf", step=epoch)
                log_writer.update(test_acer=test_stats['acer'], head="perf", step=epoch)
            ...
            if args.model_ema and args.model_ema_eval:
                ...
                print(f"acer of the model EMA on {len(dataset_val)} test images: {test_stats_ema['acer']:.3f}%")
                if min_acer_ema >= test_stats_ema["acer"]:
                    min_acer_ema = test_stats_ema["acer"]
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch="best-ema", model_ema=model_ema)
                    print(f'Min EMA acer: {min_acer_ema:.3f}%')
                if log_writer is not None:
                    log_writer.update(test_acc1_ema=test_stats_ema['acc1'], head="perf", step=epoch)
                    log_writer.update(test_acer_ema=test_stats_ema['acer'], head="perf", step=epoch)
